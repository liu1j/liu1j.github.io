...
`skills.js` 的 `entries` 列表维护 17 项技能（中文名、英文名、素材文件、可选单色标记与语言徽记）；图标保存在 `assets/skills/`，来源及许可证见该目录的 `SOURCES.md`。不依赖外部图标 CDN。`skills.css` 控制工具箱材质、手机／桌面布局和展开动效。

工具箱为独立的第 03 章，位于经历与生活板块之间，导航栏支持定位和当前章节高亮，首次进入视野自动展开一次；点击箱子重复收放，点击图标显示对应名称，Esc 收起，键盘可操作。关闭时图标退出 Tab 顺序；“减少动态效果”模式不播放位移动画。添加技能时须同时调整 `layout()` 的位置数组并检查小屏边界。
